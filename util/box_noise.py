...
def apply_box_noise(batch_boxes: torch.Tensor, infos: List[Dict], args, phase: str) -> torch.Tensor:
    """
    Apply deterministic box noise to normalized boxes (cx, cy, w, h).
    Noise is sampled per-clip using a stable seed from (global_seed, vid, sid, fids).
    """
    ...
    for b in range(bsz):
        ...
        # Center offset scaled by box size (relative noise)
        cx_new = cx + eps_center[:, :, 0] * center_std * w
        cy_new = cy + eps_center[:, :, 1] * center_std * h

        # Size scaling (log-space) + aspect-ratio perturbation (log-space)
