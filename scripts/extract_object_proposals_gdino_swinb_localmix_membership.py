...
1) Raw proposal generation:
   - LocalMix full mode(default): global branch runs Pack D
     (service/counter anchors), local branch runs Pack A/B/C/E on
     merged person-centric local crops.
   - rescue mode: global full packs first; local A/B/C only when A/B/C misses.
   - off mode: identical to original global-only extraction.
...
ALIAS_TO_TOKEN = {
    ...
    "shared dining item on table": "food container",
    ...
    "shared dining table": "table",
    "shared study table": "table",
    "dining table": "table",
    "study table": "table",
    "communal table": "table",
    "table surface": "table",
    "tabletop": "table",
    "table": "table",
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Output pickle path. Default: <data_root>/object_tracks_gdino_swinb_localmix_membership.pkl",
    )
    parser.add_argument(
        "--output_meta",
        type=str,
        default="",
        help="Output meta json path. Default: <data_root>/object_tracks_gdino_swinb_localmix_membership_meta.json",
    )
    parser.add_argument(
        "--world_size",
        type=int,
    ...
    parser.add_argument("--box_th_e", type=float, default=0.24)
    ...
    parser.add_argument("--text_th_e", type=float, default=0.18)

    # Family caps
    parser.add_argument("--cap_phone", type=int, default=3)
    parser.add_argument("--cap_study", type=int, default=5)
    parser.add_argument("--cap_dining", type=int, default=5)
    parser.add_argument("--cap_service", type=int, default=5)
    parser.add_argument("--cap_table", type=int, default=8)

    # Global family quota (used before final score fill-up)
    parser.add_argument("--quota_phone", type=int, default=1)
    parser.add_argument("--quota_study", type=int, default=3)
    parser.add_argument("--quota_dining", type=int, default=3)
    parser.add_argument("--quota_service", type=int, default=3)
    parser.add_argument("--quota_table", type=int, default=5)
    ...
    parser.add_argument("--nms_iou_table", type=float, default=0.35)
    parser.add_argument(
    ...
    parser.add_argument("--score_mul_phone", type=float, default=0.92)
    parser.add_argument("--score_mul_study", type=float, default=1.02)
    parser.add_argument("--score_mul_dining", type=float, default=1.02)
    parser.add_argument("--score_mul_service", type=float, default=1.18)
    parser.add_argument("--score_mul_table", type=float, default=1.20)
    parser.add_argument("--token_mul_phone", type=float, default=1.00)
    parser.add_argument("--token_mul_study", type=float, default=1.08)
    parser.add_argument("--token_mul_dining", type=float, default=1.08)
    parser.add_argument("--token_mul_service", type=float, default=1.18)
    parser.add_argument("--token_mul_table", type=float, default=1.25)
...
def split_prompt_packs_for_localmix(
    ...
    local_small = [p for p in prompt_packs if p.name in {"A", "B", "C", "E"}]
    global_anchor = [p for p in prompt_packs if p.name == "D"]
    return local_small, global_anchor
...
def run_packs_for_pil_images_hf(
    ...
    if not pil_images:
        return []
    if not isinstance(model, dict) or str(model.get("backend", "")).lower() != "hf":
        raise ValueError("run_packs_for_pil_images_hf requires backend=hf model bundle.")
    ...
    for group in grouped_items:
        ...
        pair_items: List[Tuple[int, PromptPack, List[str], Image.Image]] = []
        for frame_idx, pil in enumerate(pil_images):
            for pack, labels in group:
                pair_items.append((frame_idx, pack, labels, pil))
        if not pair_items:
            continue

        max_pairs = max(1, int(max_pairs_per_forward))
        target_bs = min(max_pairs, len(pair_items))
        cur_bs = target_bs
        cursor = 0
        while cursor < len(pair_items):
            chunk = pair_items[cursor : cursor + cur_bs]
            ...
            for frame_idx, pack, labels, pil in chunk:
...
def run_localmix_for_frame(
    ...
    if local_rois:
        if isinstance(model, dict) and str(model.get("backend", "")).lower() == "hf":
...
def build_prompt_packs(args: argparse.Namespace) -> List[PromptPack]:
    return [
        ...
        PromptPack(
            name="B",
            pack_id=2,
            caption="open laptop on table . laptop in use . open book . book in hand . notebook on table . paper on table . shared study material .",
        ...
        PromptPack(
            name="C",
            pack_id=3,
            caption="cup on table . bottle on table . tray on table . plate on table . bowl on table . food container on table . shared dining item on table .",
        ...
        PromptPack(
            name="E",
            pack_id=5,
            caption="shared dining table . shared study table . dining table . study table . communal table . table surface . tabletop .",
            box_threshold=float(getattr(args, "box_th_e", 0.24)),
            text_threshold=float(getattr(args, "text_th_e", 0.18)),
        ),
    ]
...
def get_family_caps(args: argparse.Namespace) -> Dict[str, int]:
    return {
        "phone-family": int(getattr(args, "cap_phone", 3)),
        "study-family": int(getattr(args, "cap_study", 5)),
        "dining-family": int(getattr(args, "cap_dining", 5)),
        "service-family": int(getattr(args, "cap_service", 5)),
        "table-family": int(getattr(args, "cap_table", 8)),
    }
...
def get_family_quota(args: argparse.Namespace) -> Dict[str, int]:
    return {
        ...
        "study-family": int(getattr(args, "quota_study", 3)),
        "dining-family": int(getattr(args, "quota_dining", 3)),
        "service-family": int(getattr(args, "quota_service", 3)),
        "table-family": int(getattr(args, "quota_table", 5)),
    }
...
def get_family_nms_iou(args: argparse.Namespace) -> Dict[str, float]:
    return {
        ...
        "table-family": float(getattr(args, "nms_iou_table", 0.35)),
    }


def get_family_score_multipliers(args: argparse.Namespace) -> Dict[str, float]:
    return {
        "phone-family": float(getattr(args, "score_mul_phone", 0.92)),
        "study-family": float(getattr(args, "score_mul_study", 1.02)),
        "dining-family": float(getattr(args, "score_mul_dining", 1.02)),
        "service-family": float(getattr(args, "score_mul_service", 1.18)),
        "table-family": float(getattr(args, "score_mul_table", 1.20)),
    }


def get_token_score_multipliers(args: argparse.Namespace) -> Dict[str, float]:
    study_mul = float(getattr(args, "token_mul_study", 1.08))
    dining_mul = float(getattr(args, "token_mul_dining", 1.08))
    service_mul = float(getattr(args, "token_mul_service", 1.15))
    table_mul = float(getattr(args, "token_mul_table", 1.25))
    return {
        "phone": float(getattr(args, "token_mul_phone", 1.00)),
        "laptop": study_mul,
        "book": study_mul,
        "notebook": study_mul,
        "paper": study_mul,
...
def _is_counter_or_table_token(token: str) -> bool:
    ...
    return ("counter" in t) or ("table" in t)
...
def resolve_base_outputs(args: argparse.Namespace) -> Tuple[Path, Path]:
    ...
    if args.output_pkl:
        ...
        base_output_pkl = data_root / "object_tracks_gdino_swinb_localmix_membership.pkl"

    if args.output_meta:
        base_output_meta = Path(args.output_meta)
    else:
        base_output_meta = data_root / "object_tracks_gdino_swinb_localmix_membership_meta.json"
...
def main() -> None:
    ...
    if local_mode != "off" and model_backend != "hf":
        print(
            "[warn] Local crop branch currently uses HF in-memory inference. "
            "For backend=official, local A/B/C falls back to full-image inference."
        )
