...
1) Raw proposal generation:
   - LocalMix full mode(default): global branch runs Pack D (service/counter),
     local branch runs Pack A/B/C on merged person-centric local crops.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Output pickle path. Default: <data_root>/object_tracks_gdino_swinb_localmix.pkl",
    )
    parser.add_argument(
        "--output_meta",
        type=str,
        default="",
        help="Output meta json path. Default: <data_root>/object_tracks_gdino_swinb_localmix_meta.json",
    )
    parser.add_argument(
        "--world_size",
        type=int,
...
def resolve_base_outputs(args: argparse.Namespace) -> Tuple[Path, Path]:
    ...
    if args.output_pkl:
        ...
        base_output_pkl = data_root / "object_tracks_gdino_swinb_localmix.pkl"

    if args.output_meta:
        base_output_meta = Path(args.output_meta)
    else:
        base_output_meta = data_root / "object_tracks_gdino_swinb_localmix_meta.json"
