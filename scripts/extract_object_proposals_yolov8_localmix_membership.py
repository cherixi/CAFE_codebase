...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Output pickle path. Default: <data_root>/object_tracks_yolov8_localmix_membership.pkl",
    )
    parser.add_argument(
        "--output_meta",
        type=str,
        default="",
        help="Output meta json path. Default: <data_root>/object_tracks_yolov8_localmix_membership_meta.json",
    )
    parser.add_argument(
        "--world_size",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "rescue=run local A/B/C only when fallback is needed, "
            "full=global E + local A/B/C."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument("--box_th_e", type=float, default=0.30)
    ...
    parser.add_argument("--text_th_e", type=float, default=0.22)
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.52,
        help="Merge highly-overlapping table-family boxes into one table box when IoU >= this threshold.",
    )
    parser.add_argument(
        "--table_merge_containment_thr",
        type=float,
        default=0.60,
        help="Also merge table-family boxes when intersection / smaller_area >= this containment threshold.",
    )
...
def split_prompt_packs_for_localmix(
    ...
    local_packs = [p for p in prompt_packs if p.name in {"A", "B", "C"}]
    global_anchor = [p for p in prompt_packs if p.name == "E"]
    return local_packs, global_anchor
...
def run_packs_for_pil_images_hf(
    ...
    return run_yolo_on_pil_images(
        ...
        class_to_token=YOLO_CLASS_TO_TOKEN,
...
def build_prompt_packs(args: argparse.Namespace) -> List[PromptPack]:
    return [
        ...
        PromptPack(
            name="E",
            pack_id=5,
            caption="yolo-pack-e-table",
            box_threshold=float(getattr(args, "box_th_e", 0.30)),
            text_threshold=float(getattr(args, "text_th_e", 0.22)),
        ),
    ]
...
def apply_table_box_merging(
    ...
    if not proposals:
        return []
    if iou_thr <= 0.0 and containment_thr <= 0.0:
        return sorted(proposals, key=lambda p: float(p.get("rank_score", p["score"])), reverse=True)

    table_items = [dict(p) for p in proposals if p.get("family") == "table-family"]
    other_items = [dict(p) for p in proposals if p.get("family") != "table-family"]
    if len(table_items) <= 1:
        kept = other_items + table_items
        kept.sort(key=lambda p: float(p.get("rank_score", p["score"])), reverse=True)
        return kept

    table_items = sorted(
        table_items,
        key=lambda p: float(p.get("rank_score", p["score"])),
        reverse=True,
    )
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            pj = table_items[j]
            iou = _iou(pi, pj)
            inter = _intersection_area(pi, pj)
            area_i = _box_area(pi)
            area_j = _box_area(pj)
            smaller = min(area_i, area_j)
            containment = (inter / max(smaller, 1e-12)) if smaller > 0.0 else 0.0
    ...
    for i in range(n):
        ...
        cluster = [table_items[idx] for idx in comp_idx]
        if len(cluster) == 1:
            merged_tables.append(cluster[0])
            continue
        ...
        merged = dict(base)
        merged["x1"] = float(min(float(p["x1"]) for p in cluster))
        merged["y1"] = float(min(float(p["y1"]) for p in cluster))
        merged["x2"] = float(max(float(p["x2"]) for p in cluster))
        merged["y2"] = float(max(float(p["y2"]) for p in cluster))
        merged["score"] = float(max(float(p["score"]) for p in cluster))
        if "rank_score" in merged:
            merged["rank_score"] = float(max(float(p.get("rank_score", p["score"])) for p in cluster))
        merged["merged_count"] = int(len(cluster))
        box = _sanitize_xyxy(merged["x1"], merged["y1"], merged["x2"], merged["y2"])
        if box is None:
            merged_tables.append(base)
        else:
            merged["x1"], merged["y1"], merged["x2"], merged["y2"] = box
            merged_tables.append(merged)

    kept = other_items + merged_tables
    kept.sort(key=lambda p: float(p.get("rank_score", p["score"])), reverse=True)
    return kept
...
def resolve_base_outputs(args: argparse.Namespace) -> Tuple[Path, Path]:
    ...
    if args.output_pkl:
        ...
        base_output_pkl = data_root / "object_tracks_yolov8_localmix_membership.pkl"

    if args.output_meta:
        base_output_meta = Path(args.output_meta)
    else:
        base_output_meta = data_root / "object_tracks_yolov8_localmix_membership_meta.json"
...
def main() -> None:
    ...
    for clip in clip_bar:
        ...
        while frame_idx < len(clip_frames):
            ...
            for fid, (image_source, raw_props), local_rois, local_info in zip(
                ...
                table_merged_props = apply_table_box_merging(
                    contain_props,
                    iou_thr=args.table_merge_iou_thr,
                    containment_thr=args.table_merge_containment_thr,
                )

                capped_props = apply_family_caps(table_merged_props, family_caps)
    ...
    meta = {
        ...
        "localmix": {
            "mode": local_mode,
            "global_branch_packs": [p.name for p in global_service_packs],
