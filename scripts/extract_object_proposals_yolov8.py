#!/usr/bin/env python3
"""Offline object proposal extraction for CAFE using YOLOv8."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract offline object proposals from CAFE using YOLOv8."
    )
    parser.add_argument(
        "--data_root",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Output pickle path. Default: <data_root>/object_tracks_yolov8.pkl",
    )
    parser.add_argument(
        "--output_meta",
        type=str,
        default="",
        help="Output meta json path. Default: <data_root>/object_tracks_yolov8_meta.json",
    )
    parser.add_argument(
        "--world_size",
        type=int,
...
def resolve_base_outputs(args: argparse.Namespace) -> Tuple[Path, Path]:
    ...
    if args.output_pkl:
        ...
        base_output_pkl = data_root / "object_tracks_yolov8.pkl"

    if args.output_meta:
        base_output_meta = Path(args.output_meta)
    else:
        base_output_meta = data_root / "object_tracks_yolov8_meta.json"
...
def main() -> None:
    ...
    for clip in clip_bar:
        ...
        use_frame_batch = model_backend in {"hf", "yolo"} and args.frame_batch_size > 1
