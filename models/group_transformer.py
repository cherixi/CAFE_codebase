...
class Transformer(nn.Module):
    ...
    def forward(self, src, actor_mask, group_dummy_mask, group_embed, pos_embed, actor_embed):
        ...
        # allow shared group queries [k, c], legacy frame queries [k*t, c],
        # or batch-specific pre-fused queries [bs, t, k, c]
        ...
        elif group_embed.dim() == 4:
            group_embed = group_embed.permute(2, 0, 1, 3).reshape(-1, bs * t, c)     # [k, bs*t, c]
        else:
            group_embed = group_embed.reshape(-1, t, c)
            group_embed = group_embed.unsqueeze(1).repeat(1, bs, 1, 1).reshape(-1, bs * t, c)
